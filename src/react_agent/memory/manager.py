            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if "locked" in str(e).lower() and attempt < max_attempts - 1:
                        logger.warning(
                            f"🧠 [MemoryManager] Database locked, retry {attempt + 1}/{max_attempts}"
                        )
...
class MemoryManager:
    ...
    def __init__(
        ...
        # Session tracking
        self.session_id = (
            session_id or f"session_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        if self.episodic_memory.current_episode:
            logger.warning(
                f" [MemoryManager] Found unclosed episode: {self.episodic_memory.current_episode.episode_id}"
            )
    ...
    def update_focus_sync(self, entities: List[str], topics: List[str]):
        ...
        if self.auto_persist and self.db_path:
            try:
                ...
                logger.warning(
                    f" [MemoryManager] Failed to persist session metadata: {e}"
                )
    ...
    async def add_tool_call(self, tool_name: str, args: Dict[str, Any], result: Any):
        ...
        for entity in entities_found:
            ...
            knowledge = {
                "type": "file"
                if any(ext in entity for ext in [".cs", ".shader"])
                else "asset",
        ...
        if query_text:
            try:
                ...
                topics_found = extract_topics_simple(
                    query=query_text, tool_name=tool_name, result_summary=result_summary
                )
                ...
                logger.error(
                    f" [MemoryManager]   ❌ Topic extraction failed: {e}", exc_info=True
                )
        ...
        if self.auto_persist and self.db_path:
            try:
                ...
                try:
                    ...
                    if (
                        len(self.semantic_memory.entity_knowledge) > 0
                        or len(self.semantic_memory.topic_knowledge) > 0
                    ):
    ...
    def find_similar_tasks(
        self, entities: List[str], topics: List[str], limit: int = 5
    ) -> List[Episode]:
    ...
    def find_relevant_patterns(
        ...
        return self.semantic_memory.find_relevant_patterns(
            context, pattern_type, min_confidence, limit
        )
    ...
    async def get_memory_context(
        self, include_patterns: bool = True, include_episodes: bool = True
    ) -> str:
        """
        ...
        if (
            len(self.working_memory.recent_tool_results) == 0
            and self.db_path
            ...
            try:
                ...
                def _reload_tools():
                    ...
                    for row in cursor.fetchall():
                        try:
                            ...
                            logger.warning(
                                f" [MemoryManager] Failed to reload tool result: {e}"
                            )
                ...
                logger.warning(
                    f" [MemoryManager]   ⚠️ Could not reload tools from database: {e}"
                )
        ...
        if (
            ...
            try:
                ...
                def _reload_entities():
                    ...
                    for row in cursor.fetchall():
                        try:
                            ...
                            logger.warning(
                                f" [MemoryManager] Failed to reload entity: {e}"
                            )
                ...
                logger.warning(
                    f" [MemoryManager]   ⚠️ Could not reload entities from database: {e}"
                )

        # ✅ FIX: Reload topic knowledge from database if empty
        if (
            len(self.semantic_memory.topic_knowledge) == 0
            and self.db_path
            and self.db_path.exists()
        ):
            try:
                ...
                def _reload_topics():
                    ...
                    for row in cursor.fetchall():
                        try:
                            ...
                            logger.warning(
                                f" [MemoryManager] Failed to reload topic: {e}"
                            )
                ...
                logger.warning(
                    f" [MemoryManager]   ⚠️ Could not reload topics from database: {e}"
                )
        ...
        if has_intent or has_tool_results or has_focus_topics:
            for topic, knowledge in list(self.semantic_memory.topic_knowledge.items())[
                :5
            ]:
                details = []
        ...
        if include_episodes and self.working_memory.focus_entities:
            ...
            if similar_episodes:
                ...
                for i, episode in enumerate(similar_episodes, 1):
                    ...
                    episode_text += (
                        f"{i}. {status}: {episode.task_description[:80]}...\n"
                    )
                    if episode.outcome_summary:
                        episode_text += (
                            f"   Outcome: {episode.outcome_summary[:100]}...\n"
                        )
        ...
        if len(self._persisted_tool_hashes) == 0 and self.db_path.exists():
            try:
                ...
                if len(self._persisted_tool_hashes) > 0:
                    logger.info(
                        f"🧠 [MemoryManager] Loaded {len(self._persisted_tool_hashes)} existing tool result hashes"
                    )
            except Exception as e:
                logger.warning(
                    f"🧠 [MemoryManager] Could not load persisted hashes: {e}"
                )
        ...
        try:
            ...
            for tool_result in self.working_memory.recent_tool_results:
                try:
                    ...
                    logger.warning(
                        f"🧠 [MemoryManager] Failed to persist tool result: {e}"
                    )
                    continue
            ...
            if persisted_count > 0:
                logger.info(
                    f"🧠 [MemoryManager] Persisted {persisted_count} new tool results"
                )
        ...
        try:
            ...
            for entity, knowledge in self.semantic_memory.entity_knowledge.items():
                try:
                    ...
                    logger.warning(
                        f"🧠 [MemoryManager] Failed to persist entity {entity}: {e}"
                    )
            ...
            for topic, knowledge in self.semantic_memory.topic_knowledge.items():
                try:
                    ...
                    logger.warning(
                        f"🧠 [MemoryManager] Failed to persist topic {topic}: {e}"
                    )
            ...
            logger.info(
                f"🧠 [MemoryManager] Persisted {entity_count} entities, {topic_count} topics to database"
            )
    ...
    def _persist_working_memory_with_cursor(self, cursor):
        ...
        if persisted_count > 0:
            logger.info(
                f"🧠 [MemoryManager]   Persisted {persisted_count} tool results"
            )
    ...
    def _persist_semantic_memory_with_cursor(self, cursor):
        ...
        if entity_count > 0 or topic_count > 0:
            logger.info(
                f" [MemoryManager]   Persisted {entity_count} entities, {topic_count} topics"
            )
    ...
    def _persist_session_metadata_with_cursor(self, cursor):
        ...
        if (
            not self.working_memory.focus_entities
            and not self.working_memory.focus_topics
            and not self.working_memory.user_intent
        ):
            logger.debug(
                " [MemoryManager] Skipping session persist - no meaningful data"
            )
            return
        ...
        try:
            ...
            logger.info(
                f" [MemoryManager]   Persisted session metadata (focus: {len(self.working_memory.focus_entities)} entities)"
            )
    ...
    def _persist_session_metadata_sync(self):
        ...
        try:
            ...
            logger.warning(
                f"🧠 [MemoryManager] Session metadata persistence locked: {e}"
            )
        except Exception as e:
            logger.error(f"🧠 [MemoryManager] Failed to persist session metadata: {e}")
    ...
    def _persist_to_database_sync(self):
        ...
        if not self.db_path or not self.db_path.exists():
            logger.warning(
                f"🧠 [MemoryManager] Cannot persist - database not found: {self.db_path}"
            )
            return
        ...
        try:
            ...
            for episode in self.episodic_memory.recent_episodes:
                try:
                    ...
                    logger.warning(
                        f"🧠 [MemoryManager] Failed to persist episode {episode.episode_id}: {e}"
                    )
                    continue
            ...
            for pattern in self.semantic_memory.patterns.values():
                try:
                    ...
                    logger.warning(
                        f"🧠 [MemoryManager] Failed to persist pattern {pattern.pattern_id}: {e}"
                    )
                    continue

            # Persist entity knowledge
            logger.info(
                f"🧠 [MemoryManager] Persisting {len(self.semantic_memory.entity_knowledge)} entities"
            )
            for entity, knowledge in self.semantic_memory.entity_knowledge.items():
                try:
                    cursor.execute(
            ...
            logger.info(
                f"🧠 [MemoryManager] Persisting {len(self.semantic_memory.topic_knowledge)} topics"
            )
            for topic, knowledge in self.semantic_memory.topic_knowledge.items():
                try:
                    cursor.execute(
            ...
            try:
                ...
                logger.warning(
                    f"🧠 [MemoryManager] Failed to persist working memory: {e}"
                )
            ...
            conn.commit()
            logger.info(
                f"🧠 [MemoryManager] Successfully persisted all memory to database"
            )
            ...
            if conn:
                try:
                    ...
                    logger.warning(
                        f"🧠 [MemoryManager] Rollback also failed: {rollback_error}"
                    )
        finally:
        ...
        try:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    logger.warning(
                        f"🧠 [MemoryManager] Failed to load tool result: {e}"
                    )

            logger.info(
                f"🧠 [MemoryManager] Loaded {len(self.working_memory.recent_tool_results)} tool results"
            )
            ...
            if session_row:
                ...
                self.working_memory.focus_entities = (
                    json.loads(session_row[1]) if session_row[1] else []
                )
                self.working_memory.focus_topics = (
                    json.loads(session_row[2]) if session_row[2] else []
                )
            ...
            try:
                entity_rows = cursor.execute(
                    "SELECT entity, knowledge FROM memory_entities"
                ).fetchall()
                logger.info(
                    f"🧠 [MemoryManager] Found {len(entity_rows)} entities in database"
                )
                ...
                for row in entity_rows:
                    try:
                        ...
                        logger.warning(
                            f"🧠 [MemoryManager] Failed to load entity knowledge: {e}"
                        )

                logger.info(
                    f"🧠 [MemoryManager] Successfully loaded {len(self.semantic_memory.entity_knowledge)} entities"
                )
            except sqlite3.OperationalError as e:
                logger.warning(
                    f"🧠 [MemoryManager] memory_entities table not found: {e}"
                )
            except Exception as e:
            ...
            try:
                topic_rows = cursor.execute(
                    "SELECT topic, knowledge FROM memory_topics"
                ).fetchall()
                logger.info(
                    f"🧠 [MemoryManager] Found {len(topic_rows)} topics in database"
                )
                ...
                for row in topic_rows:
                    try:
                        ...
                        logger.warning(
                            f"🧠 [MemoryManager] Failed to load topic knowledge: {e}"
                        )

                logger.info(
                    f"🧠 [MemoryManager] Successfully loaded {len(self.semantic_memory.topic_knowledge)} topics"
                )
            except sqlite3.OperationalError as e:
    ...
    def _reload_semantic_knowledge(self):
        ...
        logger.info(
            f"🧠 [MemoryManager] Force reloading semantic knowledge from database..."
        )
        ...
        try:
            ...
            logger.info(
                f"🧠 [MemoryManager] ✅ Force reloaded: {entity_count} entities, {topic_count} topics"
            )
    ...
    def _reload_working_memory_sync(self):
        ...
        if not self.db_path or not self.db_path.exists():
            logger.warning(
                f"🧠 [MemoryManager] Cannot reload working memory: database not found"
            )
            return

        logger.info(
            f"🧠 [MemoryManager] Force reloading working memory from database..."
        )
        ...
        try:
            ...
            logger.info(
                f"🧠 [MemoryManager] ✅ Force reloaded: {tool_count} tool results"
            )
            ...
            if session_row:
                ...
                logger.info(
                    f"🧠 [MemoryManager]     focus_entities: {self.working_memory.focus_entities}"
                )
                logger.info(
                    f"🧠 [MemoryManager]     focus_topics: {self.working_memory.focus_topics}"
                )
            else:
                logger.warning(
                    f"🧠 [MemoryManager]   ⚠️ No session metadata found in movesia.db for session {self.session_id}"
                )
