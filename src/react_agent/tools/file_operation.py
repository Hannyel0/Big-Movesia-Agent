...
async def _resolve_file_path(
    ...
    if sqlite_path and sqlite_path.exists():
        db_result = await asyncio.to_thread(
            _query_database_for_file, partial_path, project_root, sqlite_path, operation
        )
    ...
    search_result = await asyncio.to_thread(
        _search_filesystem, partial_path, project_root
    )
...
def _is_complete_path(path: str) -> bool:
    ...
    has_assets_prefix = path_lower.startswith("assets/") or path_lower.startswith(
        "assets\\"
    )
    has_extension = any(
...
def _query_database_for_file(
    filename: str, project_root: Path, sqlite_path: Path, operation: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        for row in results:
            ...
            candidates.append({"path": path, "name": name, "kind": kind, "size": size})
        ...
        exact_matches = [
            c
            for c in candidates
            if c["name"].lower() == clean_name.lower()
            or c["name"].lower() == f"{clean_name}.cs".lower()
        ]
...
def _apply_unity_heuristics(
    filename: str, project_root: Path, operation: str
) -> Dict[str, Any]:
    """Apply Unity project structure heuristics to guess file location."""
    # Clean filename
    clean_name = filename.strip()
    if not any(
        clean_name.endswith(ext) for ext in [".cs", ".unity", ".prefab", ".asset"]
    ):
...
def _search_filesystem(filename: str, project_root: Path) -> Dict[str, Any]:
...
async def _file_modify_prepare(
    abs_path: Path, rel_path: str, spec: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    # ✅ NEW: Handle full replacement
        ...
        logger.error(
            f"✏️ [FileOp] Unknown modification spec format: {list(spec.keys())}"
        )
        return {
            "success": False,
...
async def _file_delete_prepare(abs_path: Path, rel_path: str) -> Dict[str, Any]:
    ...
    if not is_file:
        return {
            "success": False,
            "error": f"Path is not a file (might be a directory): {rel_path}",
        }
...
async def _file_move_prepare(
    abs_path: Path, new_abs_path: Path, rel_path: str, new_rel_path: str
) -> Dict[str, Any]:
...
async def _execute_write(pending_op: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to write file: {type(e).__name__}: {str(e)}",
        }
...
async def _execute_modify(pending_op: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        await asyncio.to_thread(abs_path.write_text, content, encoding="utf-8")
        return {"success": True, "operation": "modify", "file_path": rel_path}
