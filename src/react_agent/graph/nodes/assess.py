...
def _extract_detailed_tool_errors(tool_result: dict, tool_name: str) -> Dict[str, Any]:
    ...
    if tool_name == "compile_and_test":
        ...
        if isinstance(warnings_data, list):
            ...
            logger.info(
                f"Found {warnings_data} warnings (count only, details might be nested)"
            )
        ...
        error_count = len(
            [
                e
                for e in detailed_context["extracted_errors"]
                if e["severity"] == "error"
            ]
        )
        ...
        if detailed_context["extracted_errors"]:
            ...
            if error_types:
                detailed_context["error_summary"] = (
                    f"{error_count} errors in {len(error_files)} files: {', '.join(error_types)}"
                )
            else:
                detailed_context["error_summary"] = (
                    f"{error_count} compilation errors found"
                )
        else:
        ...
        if error_msg:
            ...
            detailed_context["error_count"] = 1
            detailed_context["error_summary"] = (
                f"Project search error: {error_msg[:50]}"
            )
...
def _should_trigger_micro_retry(
    tool_result: dict, tool_name: str, retry_count: int
) -> bool:
    """
...
def _is_tool_result_successful(tool_result: dict, tool_name: str) -> bool:
    ...
    if tool_name == "search_project":
        return (
            bool(tool_result.get("results")) and tool_result.get("result_count", 0) > 0
        )
    elif tool_name == "code_snippets":
        return (
            bool(tool_result.get("snippets")) and tool_result.get("total_found", 0) > 0
        )
    elif tool_name in [
        "read_file",
        "write_file",
        ...
        return (
            bool(tool_result.get("results")) and len(tool_result.get("results", [])) > 0
        )
...
async def assess(state: State, runtime: Runtime[Context]) -> Dict[str, Any]:
    ...
    if not state.plan:
        ...
        if state.memory and hasattr(state.memory, "working_memory"):
            ...
            logger.info(
                f"🧠 [ASSESS] Working memory now has {len(recent_tools)} tool results"
            )
        ...
        updated_completed_steps = (
            list(state.completed_steps) if state.completed_steps else []
        )
    ...
    logger.info(
        f"🔍 [ASSESS] Plan mode - assessing step {state.step_index + 1}/{len(state.plan.steps)}"
    )
    ...
    if tool_result:
        ...
        assessment = AssessmentOutcome(
            outcome="retry", reason="No tool result found", confidence=0.5
        )
        return {
            "current_assessment": assessment,
            "total_assessments": state.total_assessments + 1,
    ...
    if _should_trigger_micro_retry(tool_result, tool_name, state.retry_count):
        ...
        logger.info(
            f"🔍 [ASSESS] Triggering micro-retry for transient error: {error_message[:80]}"
        )
    ...
    if tool_looks_successful:
        ...
        if tool_error or detailed_error_context["error_count"] > 0:
            ...
            if detailed_error_context["error_count"] > 0:
                ...
                logger.warning(
                    f"🔍 [ASSESS] Tool failed with {detailed_error_context['error_count']} errors"
                )
            else:
            ...
            assessment = AssessmentOutcome(
                outcome="retry", reason=reason, confidence=0.8
            )

            # Check if this should trigger error recovery (non-transient errors)
            if _should_trigger_error_recovery(
                tool_result, tool_error, detailed_error_context
            ):
                logger.info(
                    f"🔍 [ASSESS] Triggering error recovery for {detailed_error_context['error_count']} non-transient errors"
                )

                # Mark step as FAILED in plan
                updated_steps = list(state.plan.steps)
                failed_step = updated_steps[state.step_index]
                updated_steps[state.step_index] = PlanStep(
                    ...
                    error_messages=failed_step.error_messages + [reason],
                )
                ...
                # Emit plan UI update for FAILED status (reuse existing ID)
                message_id = state.messages[-1].id if state.messages else ""

                plan_ui_msg = create_plan_ui_message(
                    failed_plan, message_id, ui_message_id=state.plan_ui_message_id
                )
    ...
    if (
        ...
        and state.plan
        and state.step_index < len(state.plan.steps)
    ):
        updated_steps = list(state.plan.steps)
    ...
    if assessment.outcome == "success":
        ...
        if is_final_step:
            completion_summary = await _generate_completion_summary(
                state, model, context
            )
            ...
            transition = (
                f"✅ Step completed successfully! Moving on to: {next_step.description}"
            )
    ...
    if state.memory:
        ...
        if assessment.outcome == "success" and state.memory:
            logger.info(
                f"🧠 [ASSESS] Success recorded: {current_step.tool_name} works for '{current_step.description[:40]}'"
            )
        ...
        if assessment.outcome == "retry" and detailed_error_context["error_count"] > 0:
            ...
            logger.info(
                f"🧠 [ASSESS] Recorded {detailed_error_context['error_count']} errors in memory"
            )
        ...
        if insights["relevant_patterns"]:
            for pattern in insights["relevant_patterns"]:
                if pattern["success_rate"] < 0.3:  # This approach usually fails
                    logger.info(
                        f"🧠 [ASSESS] Memory shows low success rate ({pattern['success_rate']:.0%}) for pattern: {pattern['context'][:60]}"
                    )
    ...
    if assessment.outcome == "retry":
        logger.info(
            f"🔍 [ASSESS] Retry count: {state.retry_count + 1}/{state.max_retries_per_step}"
        )
    ...
    if assessment.outcome == "success":
        ...
        if state.step_index not in updated_completed_steps:
            ...
            logger.info(
                f"🔍 [ASSESS] Added step {state.step_index} to completed_steps list"
            )
...
def _should_trigger_error_recovery(
    tool_result: dict, error_message: str, detailed_error_context: Dict[str, Any]
) -> bool:
    ...
    if not tool_result.get("success", True):  # Explicit failure
        ...
        critical_patterns = [
            ...
            "reference error",
            "cs0103",
            "cs0246",  # Specific C# compilation errors
        ]
    ...
    for i, step in enumerate(state.plan.steps):
        if i <= state.step_index:
            completed_steps_summary.append(
                f"Step {i + 1}: {step.description} (using {step.tool_name})"
            )
                ...
                if _is_tool_result_successful(result, tool_name):
                    if tool_name == "file_operation":
                        ...
                        recent_tool_results.append(
                            f"Found {count} Unity documentation pages"
                        )
                    else:
                        recent_tool_results.append(
                            f"{tool_name}: {result.get('message', 'completed successfully')}"
                        )
            except:
...
Generate a brief, professional completion message (1-2 sentences) that starts with "Perfect!" and summarizes what was accomplished."""
    ...
    try:
        ...
        if len(completion_summary) < 20 or not completion_summary.lower().startswith(
            ("perfect", "excellent", "great", "done", "completed", "success")
        ):
            return (
                f"Perfect! I've successfully completed your request: {state.plan.goal}"
            )
