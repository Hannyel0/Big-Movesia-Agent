...
class RecoveryStep(BaseModel):
    ...
    error_context: Optional[str] = Field(
        default="", description="Specific error context this step addresses"
    )
...
class ContextAwareErrorRecovery(BaseModel):
    ...
    fallback_approach: str = Field(
        default="", description="Alternative approach if fix fails"
    )
    estimated_complexity: str = Field(
        description="Simple, moderate, or complex recovery"
    )
...
def extract_detailed_error_context(
    tool_result: Dict[str, Any], tool_name: str
) -> List[Dict[str, Any]]:
            ...
            detailed_errors.append(
                {
                    "type": "file_error",
                    "tool": tool_name,
                    "file": tool_result.get("file_path", "")
                    or tool_result.get("attempted_path", ""),
                    "error": error_msg,
                    "severity": "error",
                }
            )
...
def create_context_aware_template_recovery(
    ...
    if error_category == ErrorCategory.BUILD_ERROR and error_count > 1:
        ...
        logger.info(
            f"Created context-aware recovery for {error_count} build errors: {len(recovery_steps)} steps"
        )
    ...
    # Single error or other categories - use simple template
        ...
        if template:
            ...
            logger.info(
                f"Created template recovery for single {error_category}: 2 steps"
            )
...
async def diagnose_error(
    ...
    logger.info(
        f"Extracted {error_count} specific errors: {[e.get('error', '')[:50] for e in specific_errors]}"
    )

    # Quick categorization
    category = _categorize_error_fast(error_message, tool_name)
    severity = _assess_severity_fast(
        error_message, tool_name, state.retry_count, error_count
    )
    ...
    if specific_errors:
        error_summary = "; ".join(
            [
                f"{e.get('file', tool_name)}: {e.get('error', '')[:50]}"
                for e in specific_errors[:3]
            ]
        )
        description = (
            f"Tool {tool_name} failed with {error_count} errors: {error_summary}"
        )
        ...
        if category == ErrorCategory.BUILD_ERROR:
            ...
            root_cause = (
                f"Compilation errors: {', '.join(set(error_types))}"
                if error_types
                else f"{error_count} compilation errors requiring fixes"
            )
        else:
...
async def create_error_recovery_plan(
    ...
    if diagnosis.category in CONTEXT_AWARE_TEMPLATE_FIXES:
        logger.info(
            f"Using Tier 1 context-aware template recovery for {diagnosis.category} with {diagnosis.error_count} errors"
        )
        template_recovery = create_context_aware_template_recovery(
            diagnosis.category, failed_step, diagnosis.specific_errors
        )
    ...
    logger.info(
        f"Using Tier 2 enhanced LLM recovery for {diagnosis.category} with {diagnosis.error_count} errors"
    )
...
Create a recovery plan that addresses the specific errors listed above, not just the general error category."""
    ...
    try:
        ...
        structured_model = model.with_structured_output(
            ContextAwareErrorRecovery, method="function_calling"
        )
        recovery_response = await structured_model.ainvoke(
            [
                {
                    "role": "system",
                    "content": f"Create targeted recovery plans that address ALL specific errors. For {diagnosis.error_count} errors, create appropriate steps to fix each issue. Always end with verification.",
                },
        ...
        if len(recovery_steps) > max_steps:
            logger.warning(
                f"LLM returned {len(recovery_steps)} steps, truncating to {max_steps}"
            )
        ...
        if diagnosis.category == ErrorCategory.BUILD_ERROR and recovery_steps:
            ...
            if last_step.tool_name != "compile_and_test":
                ...
                recovery_steps[-1] = PlanStep(
                    ...
                    tool_name="compile_and_test",
                    success_criteria="Clean compilation with no errors",
                    dependencies=[len(recovery_steps) - 2]
                    if len(recovery_steps) > 1
                    else [],
                )

        logger.info(
            f"Created enhanced LLM recovery: {len(recovery_steps)} steps for {diagnosis.error_count} errors"
        )
...
def _create_context_aware_fallback_recovery(
    failed_step: PlanStep, diagnosis: ErrorDiagnosis
) -> List[PlanStep]:
...
async def execute_error_recovery(
    ...
    logger.info(
        f"⚠️ [ERROR_RECOVERY] Step {state.step_index + 1}/{len(state.plan.steps) if state.plan else 0}"
    )
    ...
    if state.memory:
        ...
        error_patterns = [
            p
            for p in insights.get("relevant_patterns", [])
            if "error" in p.get("context", "").lower()
            or "fail" in p.get("context", "").lower()
        ]

        if error_patterns:
            logger.info(
                f"🧠 [ERROR_RECOVERY] Found {len(error_patterns)} similar error patterns in memory"
            )
            for pattern in error_patterns[:3]:  # Log top 3
                logger.info(
                    f"🧠 [ERROR_RECOVERY]   - {pattern.get('context', 'unknown')[:60]}: {pattern.get('success_rate', 0):.0%} success rate"
                )
        else:
            logger.info(
                f"🧠 [ERROR_RECOVERY] No similar error patterns found in memory"
            )

    # Create context-aware recovery plan
    recovery_steps = await create_error_recovery_plan(
        diagnosis, current_step, state, runtime
    )

    if not recovery_steps:
        logger.warning("⚠️ [ERROR_RECOVERY] No recovery steps created - using fallback")
        recovery_steps = _create_context_aware_fallback_recovery(
    ...
    updated_plan = _replace_failed_step_with_recovery(
        state.plan, state.step_index, recovery_steps
    )
    ...
    recovery_message = _create_detailed_recovery_message(
        diagnosis, recovery_steps, state.retry_count
    )

    logger.info(
        f"⚠️ [ERROR_RECOVERY] Recovery strategy: {diagnosis.error_count} errors, {len(recovery_steps)} steps"
    )
...
def _replace_failed_step_with_recovery(
    original_plan: ExecutionPlan, failed_step_index: int, recovery_steps: List[PlanStep]
) -> ExecutionPlan:
    ...
    logger.info(
        f"Plan updated: {len(original_plan.steps)} → {len(new_steps)} steps (replaced with {len(recovery_steps)})"
    )
...
def _create_detailed_recovery_message(
    diagnosis: ErrorDiagnosis, recovery_steps: List[PlanStep], retry_count: int
) -> str:
...
def _categorize_error_fast(error_message: str, tool_name: str) -> ErrorCategory:
    ...
    if any(term in error_lower for term in ["not found", "missing", "does not exist"]):
        ...
        term in error_lower for term in ["config", "setting", "invalid parameter"]
    ):
        ...
        term in error_lower
        for term in ["compile", "build", "syntax", "cs0103", "cs0246"]
    ):
...
def _assess_severity_fast(
    error_message: str, tool_name: str, retry_count: int, error_count: int = 1
) -> ErrorSeverity:
