...
async def advance_step(state: State, runtime: Runtime[Context]) -> Dict[str, Any]:
    ...
    if state.step_index >= len(state.plan.steps):
        return {
            "messages": [AIMessage(content="ERROR: Step index beyond plan bounds.")]
        }
    ...
    # Emit updated plan to UI (reuse existing ID for in-place update)
    message_id = (
        progress_message.id
        if progress_message
        else (state.messages[-1].id if state.messages else "")
    )

    # CRITICAL: Reuse existing plan_ui_message_id for in-place updates
    plan_ui_msg = create_plan_ui_message(
        updated_plan, message_id, ui_message_id=state.plan_ui_message_id
    )

    # CRITICAL FIX: Persist plan_ui_message_id in state update
    result = {
        "plan": updated_plan,
        "plan_ui_message_id": plan_ui_msg.id,  # ✅ CRITICAL: Persist the ID!
...
async def increment_retry(state: State, runtime: Runtime[Context]) -> Dict[str, Any]:
    ...
    return {"retry_count": state.retry_count + 1, "current_assessment": None}
